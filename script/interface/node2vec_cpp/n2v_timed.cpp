...
void node2vec(PWNet& InNet, const double& ParamP, const double& ParamQ,
  ...
  std::time(&toc);
  duration = double (toc - tic);
  printf("Took %02d:%02d:%05.2f to pre-compute transition probabilities\n", \
    (int) duration / 3600, (int) duration % 3600 / 60, duration - (int) duration / 60 * 60);
...
#pragma omp parallel for schedule(dynamic)
  ...
  std::time(&toc);
  duration = double (toc - tic);
  printf("Took %02d:%02d:%05.2f to generate walks\n", \
    (int) duration / 3600, (int) duration % 3600 / 60, duration - (int) duration / 60 * 60);
  //Learning embeddings
  std::time(&tic);
  if (!OutputWalks) {
  ...
  std::time(&toc);
  duration = double (toc - tic);
  printf("Took %02d:%02d:%05.2f to train embeddings\n", \
    (int) duration / 3600, (int) duration % 3600 / 60, duration - (int) duration / 60 * 60);
}
